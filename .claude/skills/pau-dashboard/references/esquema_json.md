...
- **cuestion.figura_enunciado_svg**: figura del enunciado. Normalmente un `<svg>` reproducido con fidelidad (ver `figuras.md`). Si la figura es demasiado compleja para vectorizar bien, admite también un `<img src="fig/imagen_03_01.png" style="max-width:100%">` apuntando a la imagen original extraída con `extract_figures.py` (guárdala junto al dashboard). Vacío si no hay figura.
